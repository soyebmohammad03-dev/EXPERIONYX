...
## Schema versioning
`PRAGMA user_version` holds `DB_SCHEMA_VERSION` (currently 15; v15 added `resource_analyses` and `resource_trials`; v14 added `calibration_analyses` and `calibration_results`; v13 added `stress_analyses` and `stress_trials`; v12 added `quality_analyses` and `quality_checks`; v11 added `temporal_windows` and `drift_analyses`; v10 added `slices` and `slice_analyses`; v9 added `statistical_analyses`; v8 added `benchmarks`, `benchmark_results` and `benchmark_units`; v7 added `reliability_profiles` and `reliability_references`; v6 added `interaction_analyses`, `interaction_effects` and `interaction_evidence`; v4 added the fault-laboratory tables, v5 the failure tables `failure_signals`, `failure_clusters`, `failure_modes`, `failure_evidence`, `failure_relationships`): v2 added `provenance`/`outcomes`
