...
| `experionyx resources validate\|list\|inspect\|run\|sweep\|compare\|replay …` | real latency, throughput, CPU and memory measurement of a registered model on a registered dataset (environment-specific engineering measurements; no composite score); `validate --preflight` refuses unsupported requests (memory limit, unsafe concurrency, unsupported stress) and invalid batch sizes before anything runs, `run`/`sweep` exit 3 if a trial failed or timed out or evidence is insufficient, `sweep` persists every batch size as its own run and compares with a multiple-comparison correction, `compare` stores nothing, `replay` compares the definition and model outputs (never the timings) and exits 1 on a difference; see [resources.md](resources.md) |
