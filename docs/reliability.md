...
`INTERACTION_SENSITIVITY`, `SLICE_SENSITIVITY`, `DISTRIBUTION_SHIFT`, `MODEL_STRESS`, `CALIBRATION_ANALYSIS`, `RESOURCE_SYSTEM`, `REPRODUCIBILITY`, `UNCERTAINTY`, `LATENCY`,
...
- **Calibration analysis** (Phase 15): copies the recorded results of referenced calibration analyses (`calibration_analyses`, `cba_`): representation, binning, baseline metrics, per-context statuses and comparison counts; `UNAVAILABLE` if none (the absence of calibration evidence is not evidence of good calibration), `INSUFFICIENT_EVIDENCE` if no analysis had enough usable observations. It is separate from the baseline's own `CALIBRATION` dimension, is never merged with `UNCERTAINTY`, and forms no score. See [calibration.md](calibration.md).
- **Resource system** (Phase 16): copies the recorded results of referenced resource analyses (`resource_analyses`, `rsa_`): workload, requested and effective batch size, worker count, trial counts, initialization and warmup, latency and throughput statistics, timeouts and failures, CPU, memory, output consistency and the environment they were measured in. `UNAVAILABLE` if none (the absence of measurements is not evidence of good or bad system behaviour), `INSUFFICIENT_EVIDENCE` if no analysis had enough completed measured trials. These are environment-specific engineering measurements: performance measurement is not reliability, it is separate from the baseline's own `LATENCY` dimension, and it forms no score. See [resources.md](resources.md).
